...
| **AB-29** ✅ **DONE (Phase 6, Slice 2)** — deleted. **The finding understated it: the file existed in BOTH backends**, byte-identical (`diff` clean), 6 lines each, with **zero** references outside itself in either service. Only the admin copy was recorded; the client twin would have been left behind by a literal reading. Deleting rather than adopting: retrofitting `asyncHandler` across ~50 hand-rolled try/catch blocks in two services is a refactor with real regression risk, and those blocks now carry `sanitizeError` calls that a generic wrapper would bypass. **Verified:** zero references remain in either backend; both boot and serve. | `middlewares/asyncHandler.js` | Implemented, exported, and **never used** anywhere. All controllers hand-roll try/catch. | Delete, or adopt it consistently. | **DONE** |
| **AB-30** ✅ **DONE (Phase 6, Slice 2)** — **7 orphaned dbOps methods + their 7 SQL strings** deleted across both backends, after confirming no dynamic dispatch (`dbOps[name]()`) exists anywhere, which is what makes the zero-call-site greps trustworthy. Six were the **pre-Phase-2 session layer**, each sitting directly above the `// --- Phase 2 auth ---` divider that replaced it: `getAdminLastSessionByEmail`, `updateToken`, `updateSessionStatus` and the client twins `getCustomerLastSessionByEmail`, `updateCustomerToken`, `updateCustomerSessionStatus`. The live path uses `getActiveSessionById` per request and `logoutSessionBySessionId` on logout — both confirmed in use and untouched. `getImagesByProductId` is **still called** and was kept; the finding was stale on that point.<br><br>⚠️ **`resetPrimaryImageByImageId` was deleted, and the note that "`AB-16` needs it" is superseded — it was a loaded gun, not a head start.** `is_primary_image` is set exactly once, at upload (`productManager.js:65`, `adminController.js:121`: `i === 0 ? 1 : 0` — first file wins), and **there is no admin UI to change it afterwards**. Meanwhile **9 storefront queries** join `AND pi.is_primary_image = 1` to pick each product's display image. The orphan ran `UPDATE product_images SET is_primary_image = 0` for **all** of a product's images — the *clear* half of a set-cover operation whose *set* half was never written. Called on its own it leaves a product with no primary image, every one of those 9 `LEFT JOIN`s returns `NULL` for `image_url`, and **the saree silently loses its picture on the shop**. Keeping three lines that break the storefront if wired naively is worse than keeping nothing; the real feature is a two-statement transaction, now recorded under Deferred work. **Verified:** both backends boot; all 8 admin GET routes 200 authenticated and 401 anonymous; products still return image URLs; full customer journey signup→login→cart→orders→logout all 200, with 401 after logout proving session invalidation still works without the removed helpers. | `controllers/adminController.js:283-298`; `routes/adminRoutes.js:45`; `dbOps/adminDbOps.js:218, :224` | `updateProductImages` is orphaned (its route is commented out). `getImagesByProductId` and `resetPrimaryImageByImageId` are never called. | Delete or wire up. ~~Note `resetPrimaryImageByImageId` is needed by `AB-16`~~ — superseded, see above. | **DONE** |
...
| **AB-37** ✅ **DONE (Phase 6, Slice 3)** — `express.urlencoded({ extended: true, limit: '100kb' })` mounted in **both** backends, right after `express.json()`. The limit mirrors `express.json`'s default rather than leaving the body unbounded. **Verified live:** an `application/x-www-form-urlencoded` admin login now returns **200** with correct credentials and **401** with wrong ones — before the change the body never parsed, so the validators reported every field missing and returned a **400 blaming the caller for a body the server never read**. Does not affect `/api/insert-image`: `multipart/form-data` is parsed by multer, mounted separately on that route. | `server.js` | No `express.urlencoded()` — any HTML form / `x-www-form-urlencoded` client gets `req.body === undefined` and hits `AB-18`. | Add `app.use(express.urlencoded({ extended: true, limit: '100kb' }))`. | **DONE** |
| **AB-38** ✅ **DONE (Phase 6, Slice 3)** — `const PORT = Number(process.env.PORT) || 9032` in admin, `|| 9034` in client; the literal is now the local-dev default only. ⚠️ **This was the one item in Phase 6 with a real go-live consequence: Render assigns the port at runtime and routes traffic ONLY there, so a hardcoded port binds where the platform is not listening.** The health check never passes and the deploy is marked failed — while the service's own logs cheerfully read "Server running on port 9032", which is what makes it slow to diagnose. **`DEPLOYMENT.md:141` already documented this** (`PORT — Render provides this, read it, don't hardcode`); the docs were right and the code simply never complied, which is the argument for CI over documentation. **Verified:** with `PORT=8471`/`PORT=8472` injected, both bound to the injected port and served (`/api/products` 200, admin 401 anon / 200 authed); with no `PORT` set both fell back to 9034/9032 and served identically, so local development is unaffected. | `server.js:25` | `const PORT = 9032` hardcoded instead of `process.env.PORT`. | `const PORT = process.env.PORT || 9032`. | **DONE** |
| **AB-39** ✅ **DONE (Phase 6, Slice 3)** — all three parts, in **both** backends.<br><br>**(a) `pool.on('error')`.** Not merely a logging gap: **an `'error'` event with no listener is re-thrown by EventEmitter as an uncaught exception**, so a dropped idle connection (`PROTOCOL_CONNECTION_LOST`, `ECONNRESET`, or MySQL's `wait_timeout` reaping an unused connection) would have **taken the whole process down** for a condition mysql2 recovers from by itself. Logs `err.code` only, never the error object, keeping SQL and bound parameters out of the logs.<br><br>**(b) `queueLimit: 0` → `50`.** ⚠️ I first recorded this as deferred on the grounds that picking a number needs load data — **that reasoning was wrong and the owner challenged it.** Unbounded is not the neutral choice, it is the worst one: a database stall does not fail, it *accumulates*. Every request waits on a connection that never frees, the queue grows with memory, and each browser spins until it times out on its own — nothing sheds load and nothing reports a problem. 50 is 5x `connectionLimit`, roughly a quarter-second of backlog at ~50ms per query, so ordinary spikes still queue and succeed while a genuine stall rejects the 51st waiter immediately as a generic 500. Tune from real traffic once live; do not return it to unbounded.<br><br>**(c) `GET /health`.** Mounted at `/health`, deliberately **not** under `/api` — everything under `/api` is rate-limited, and a platform polling every few seconds would throttle itself and report the service as down. Reports **readiness, not liveness**: it runs `SELECT 1`, because a process that is up but cannot reach MySQL serves nothing useful, and answering 200 in that state tells Render to keep routing traffic to a service that 500s every request. The body is deliberately minimal since the endpoint is unauthenticated — no database name, host, version or error text; the reason goes to the logs.<br><br>**Verified:** `/health` → `{"status":"ok","db":"up"}` **200** on both; **20 rapid consecutive hits all 200**, confirming it bypasses the limiter; and booted against an unreachable database (`DB_PORT=3999`) it returned `{"status":"degraded","db":"down"}` **503**, logging exactly `[health] database unreachable: ECONNREFUSED` and leaking nothing in the body. Normal endpoints unaffected under the new `queueLimit`. ⚠️ **Set the Render health-check path to `/health` for both services** — see `DEPLOYMENT.md`. | `config/db.js` | No `pool.on('error')` handler, no connection health check. `connectionLimit: 10` with `queueLimit: 0` (**unbounded** queue) means a DB stall queues requests without limit. | Add an error handler and a finite `queueLimit`. Add a `/health` endpoint. | **DONE** |
| **AB-40** ✅ **DONE (Phase 6, Slice 3)** — graceful shutdown and both last-resort process handlers added to **both** backends. Render sends `SIGTERM` on every deploy, restart and scale event, then `SIGKILL`s after a short grace period; with no handler the process died instantly and **cut off in-flight requests mid-response — including a checkout whose transaction had already COMMITTED but whose response never arrived**, so the customer sees a network error for an order that genuinely exists. On the admin side it can also abandon an in-flight Cloudinary upload, leaving a stored asset with no database row — the orphan class `AB-10` fixed.<br><br>Implementation notes: `server.close()` stops accepting new connections and fires once in-flight requests finish, then `pool.end()` drains the connections (without it MySQL holds up to 10 per instance until they time out, and a redeploy loop piles them up faster than they are reclaimed); a `shuttingDown` guard stops a second signal re-entering; a **10s `setTimeout(...).unref()` backstop** exits on our own terms rather than hanging on a stuck connection until Render escalates to `SIGKILL`; `SIGINT` is handled too so local Ctrl+C behaves the same. `unhandledRejection` and `uncaughtException` log via `sanitizeError` and then shut down — an unhandled rejection leaves the process in an unknown state, so restarting cleanly beats serving requests from a corrupted one. **Verified:** `kill -TERM` on both produced `[SIGTERM] shutting down` → `HTTP server closed, database pool drained`, and **both ports were confirmed released**. | `server.js` | No graceful shutdown (`SIGTERM`), no `process.on('unhandledRejection')`. | Add both. | **DONE** |
...
| **CB-37** ✅ **DONE (Phase 6, Slice 3)** — the client-backend twin of `AB-38`/`AB-39`/`AB-40`, all applied identically: `PORT` from the environment (`|| 9034` as the local default), graceful `SIGTERM`/`SIGINT` shutdown with pool drain and a 10s backstop, `unhandledRejection`/`uncaughtException` handlers, `express.urlencoded`, `pool.on('error')`, `queueLimit: 50`, and `GET /health`. `app.set('trust proxy', 1)` was already present from an earlier phase, correctly gated on `NODE_ENV === 'production'` so a local client cannot spoof `X-Forwarded-For`.<br><br>**The `dotenv` ordering hazard is now closed properly, and it nearly bit me while fixing it.** The explicit `require('dotenv').config()` existed but sat *below* the local requires; adding `const pool = require('./config/db')` for the shutdown handler put a module that reads `process.env` at import time **above** the line that populates it — recreating the exact failure this finding warns about. `dotenv.config()` is now the **first statement in the file**, above every local require, with a comment saying why nothing may be moved above it. It worked either way only because `config/db.js` also self-loads dotenv; relying on that is precisely the silent coupling the finding describes. **Verified:** with `PORT=8471` injected the service bound there and served `/api/products` 200; with no `PORT` it fell back to 9034; `kill -TERM` logged `[SIGTERM] shutting down` → `HTTP server closed, database pool drained` and released the port. | `server.js:28`; `config/db.js:2` | Port `9034` hardcoded. `dotenv.config()` is called only as a **side effect** of `require('./config/db')`. No `SIGTERM` handler, no `unhandledRejection` handler. | Explicit `dotenv.config()` at the top of `server.js`; `process.env.PORT`; add the handlers. | **DONE** |
...
| **CB-40** 🆕 ✅ **DONE (Phase 6, Slice 2)** — found while running the post-deletion verification pass, not by audit. | `middlewares/validators.js:79` (was `body('password')`); `controllers/customerController.js:14, :16, :27, :36, :43`; `client-frontend/components/Signup.jsx:98-102` | **The same field had two names across three adjacent auth endpoints.** `POST /api/signup` required `password`, while **both** `POST /api/customer-login` and `POST /api/admin-login` required `passwd`. Posting the signup body shape to login returned `400 "Password is required."` **with a perfectly correct password** — an error message that describes a missing field when the field is present, just differently named. Nothing was broken at runtime because each frontend form happened to be written against its own endpoint (`LoginPage.jsx:50` and `AdminLogin.jsx:53` both map `passwd: formData.password`; `Signup.jsx` spread `formData`, which carried `password`), so the mismatch was invisible from the UI and would only ever bite whoever integrates against this API next — or whoever writes the first test for it. | Standardised on **`passwd`**: it was already 2 of the 3 auth endpoints, and it matches the abbreviated `pri_email` convention used throughout this codebase. Only signup changed, so **neither login path was touched** — the riskier surface stayed still. Validator, controller (4 references) and the `Signup.jsx` payload were changed together in one commit; a partial change here would have locked every new customer out of registration. | **DONE** — **Verified:** signup with `passwd` → **201**; signup with the old `password` → **400** (proving the change took effect); signup → login → `verify-token` → `/me` → `logout` round-trip on a freshly created account, **all 200**, using one field name throughout; admin login **200** and a wrong password still **401**, confirming no hash or credential behaviour changed. All test accounts created during verification were deleted afterwards (`0` remaining `@example.com` rows). |
...
| **AF-08**  ✅ **DONE (Phase 5.5, Slice 4)** — both `localStorage.setItem("categoryProducts", ...)` writes removed. Nothing ever read the key back — `AdminHomePage` carried only a comment noting its reader had been deleted — so product data including prices and stock accumulated in the browser indefinitely with no invalidation.<br><br>✅ **Residual CLOSED (Phase 6, Slice 2).** The state, the prop, both pass sites in `AdminHomePage` and both call bodies in `AddProduct`/`UpdateProduct` are gone. The removal also orphaned everything that existed to feed that setter: `finalImagesForUI` in `AddProduct` (its `if (success)` branch inverted to `if (!success)` so the warning behaviour is byte-identical), and the entire "STEP B: Update UI" block in `UpdateProduct` — `responseData`, `finalImages`, and the old/new image-URL merge, none of which had any other reader. Request success is still checked by the `res.ok` guard above it, and `AllProducts` refetches from the server on return, which is why this bookkeeping never mattered.<br><br>⚠️ **I broke this before I fixed it, and the cause is worth recording.** I searched for `categoryProducts` — **case-sensitive, which does not match `setCategoryProducts`** — concluded it had one occurrence app-wide, and deleted the state while two children were still calling its setter. That would have crashed **both the Add Product and Update Product screens**. ESLint caught it (`'setCategoryProducts' is not defined` ×2). **A case-sensitive grep for a camelCase identifier will silently miss every `setX`/`handleX`/`onX` wrapper of it; use `grep -i` when establishing that something is unused.** The owner had flagged the risk before the verification run surfaced it. **Verified:** case-insensitive search returns only comments; admin frontend builds; lint **8 → 7** (the removed error is `categoryProducts`, with no new ones — the transient 9 was my half-removal). | `AddProduct.jsx:190`; `UpdateProduct.jsx:199` | `localStorage.setItem("categoryProducts", …)` is a **write-only cache** — nothing reads it anywhere (`AdminHomePage.jsx:79` is only a comment noting the reader was removed). Product data with prices and stock accumulates unbounded with no invalidation. | Delete both writes. | TODO |
...
| **AF-37** ✅ **DONE (Phase 6, Slice 2)** — deleted `App.jsx` (15 lines; no importers, and it imported `./assets/react.svg` which does not exist), `App.css`, `productData.json` (a 2-line `{"categories": []}` stub whose import at `AdminHomePage.jsx:5` was shadowed by a local `const data` everywhere it was referenced), `AdminLogin.css`, `tailwind.confing.js`, and **7** of the 8 listed assets. `adminDetails.js`, `components/OrderedProducts/*`, `RecentOrders.js` and `assets/SareeImage.png` were already absent — removed in earlier phases, so the finding was stale on those.<br><br>`AdminLogin.css` was verified before deletion rather than assumed: its selectors are `.login-page`, `.login-area`, `.admin-image-section`, `.login-credentials-section` and `.form`, and the only "form" matches in `AdminLogin.jsx` proved to be `formData` and the `<form>` tag itself — **the stylesheet matched no element**; the layout is Tailwind. `tailwind.confing.js` was triple-dead: the filename is misspelled so Tailwind never looked for it, v4 uses CSS-first `@theme` rather than a JS config anyway, and its `module.exports` would throw inside a `"type": "module"` package. The identical typo'd file existed in **client-frontend** too and went with it. **Verified:** admin frontend builds. | dead files | **DONE** — original finding text follows: `App.jsx`, `App.css`, `adminDetails.js`, `components/OrderedProducts/*`, `productData.json`, `RecentOrders.js`, `AdminLogin.css` (imported but its selectors match no element), `tailwind.confing.js`, and 8 unreferenced assets (`assets/svg/AllProducts.svg`, `DashBoard.svg`, `delete.svg`, `Notifications.svg`, `OrderList.svg`, `SareeImage.png`, `UpperArrowMark.svg`, `assets/png/DemoSaree.png`). | Delete. | TODO |
...
| **CF-34** ✅ **DONE (Phase 6, Slice 2)** — it was **8 pages, not 6**: `Homepage`, `MyOrders`, `MyProfile`, `App`, `NotFound`, `ViewProductPage`, `AboutUs`, `ForgotPassword`, plus `Header` itself. `Header` now takes **no props** and all eight render `<Header />`.<br><br>**The cascade is what made this bigger than deleting 40 lines,** and it differed per file — which is exactly why it had to be done one file at a time rather than by find-replace. Removing a prop orphaned the variable feeding it: `updateCart` existed *only* to be passed as `onUpdate`, so its definition died, which was the only reader of `setCartItems`, which then left the `useContext` destructure. In five files (`NotFound`, `ForgotPassword`, `MyProfile`, `MyOrders`, `AboutUs`) that emptied the destructure completely and took the `useContext` **and** `CartContext` imports with it — each verified to have exactly 1 `useContext(` call and 2 `CartContext` mentions beforehand. In `Homepage` and `ViewProductPage` the destructure **stayed**: `cartItems` and `wishListItems` have real readers there (`Homepage.jsx:226`/`:98`), so only the two setters came out.<br><br>⚠️ **`App.jsx` nearly broke and is the lesson here.** I had recorded "whole destructure dies" for it, inferred from grep *counts* rather than reading the file. It also destructures **`handleAddToCart`**, used at `:246`. Reading the file caught it; the count-based inference would have removed a live binding. **Nothing in this register should be acted on without opening the file first.**<br><br>**Runtime impact was, correctly, zero** — this was never a bug. The cost was that the code lied: `<Cart cartItems={cartItems} />` reads as though the cart renders from that prop, so anyone debugging a cart problem changes what `Homepage` passes and nothing happens, with no error to explain why. **Verified:** zero `cartItems=`/`onUpdate=`/`wishListItems=`/`onWishListUpdate=` remain anywhere; zero orphaned `updateCart`/`updateWishList` definitions; `CartContext.jsx` **untouched** (9,482 bytes, all four handlers, still consumed by 17 files); client frontend builds; lint count unchanged at 21, none of them in the 9 files touched. | `Header.jsx:14, :97-98, :105-106`; `Cart.jsx:11-19`; `WishList.jsx:9-14` | **~24 lines of dead prop plumbing across 6 files** — 8 pages in reality, see above. `Header` accepts `cartItems`/`onUpdate`/`wishListItems`/`onWishListUpdate` and forwards them to `Cart` and `WishList`, and **both children ignore them entirely** and read `useContext(CartContext)` directly. | Delete the props from all six call sites. | **DONE** |
...
| **CF-50** ✅ **DONE (Phase 6, Slice 2)** — all 19 CSS files deleted, plus `products.js` (328 lines), `userOrder.js` (53), `Pageloader.jsx` (41), `ui/card.jsx` (101), `ui/textarea.jsx` (20), and — not in the original finding — `ui/calendar.jsx` (178) and `ui/popover.jsx` (45), both freed by removing the commented-out DOB block (`CF-40`), plus `assets/lottie/animation.json` and the misnamed `tailwind.confing.js`. `products.js` was imported at `Homepage.jsx:12` feeding `topFourProducts` at `:45`, which was never rendered — both lines removed before deleting the file. **Verified:** client frontend clean-installs and builds. | dead code | **DONE** — original finding text below.<br><br>**19 of 20 CSS files are dead** — only `index.css` is imported (`main.jsx:6`). `App.css`, `Checkout.css`, `Homepage.css`, `MyOrders.css`, `MyProfile.css` and all 14 `components/*.css` are referenced by no `import`. A full pre-Tailwind stylesheet layer left in place, guaranteeing the next person to edit `components/Cart.css` will wonder why nothing changes. Also dead: `components/Pageloader.jsx` (superseded by `PeacockLoader`, and the **sole** reason `lottie-react` + `assets/lottie/animation.json` are installed), `components/ui/card.jsx` (101 lines), `components/ui/textarea.jsx`, `src/userOrder.js` (fabricated data, no real PII), and `src/products.js` (**328 lines of mock products with fake prices**, imported at `Homepage.jsx:9` solely to feed `topFourProducts` at `:42`, which is never rendered — tree-shaken, but one careless line away from being displayed as real). | Delete all of it. | TODO |
| **CF-51** ✅ **DONE (Phase 6, Slice 2)** — **9 removed, not 8**: the listed eight plus `@radix-ui/react-popover`, freed when the commented-out DOB field's `Popover` wrapper went. Admin-frontend separately lost `@radix-ui/react-icons` (`DEP-11`).<br><br>⚠️ **`@tailwindcss/vite` was challenged by the owner as load-bearing — it is not, and the check is worth recording.** Tailwind v4 has two *alternative* wirings: the Vite plugin (`@tailwindcss/vite`, which must be imported in `vite.config.js`) or the PostCSS plugin (`@tailwindcss/postcss`, declared in `postcss.config.js`). This project uses **PostCSS** — `vite.config.js` never imported the Vite plugin, its array is `[react(), svgr()]`. Two confirmations: the **admin frontend has zero `@tailwindcss/vite`** and Tailwind works there identically; and after deleting it from `node_modules` the build produced **82 KB of compiled Tailwind** — `.flex{`, `.grid{`, 16 `.rounded*`, 46 `md:` variants, 87 `hover:` variants, 69 instances of the `#68232B` brand colour from arbitrary-value classes. Load-bearing would have collapsed that to the handwritten `body` rules. **Verified:** both frontends clean-install and build. The Zod `z.string().email()` migration noted below was already done — zero occurrences remain. | `package.json` | **DONE** — original finding text below.<br><br>**8 unused dependencies** (verified zero imports): `@tanstack/react-table`, `@radix-ui/react-dropdown-menu`, `@radix-ui/react-progress`, `@radix-ui/react-icons`, `@tailwindcss/vite` (installed but **not** wired in `vite.config.js` — two competing Tailwind pipelines declared, one used), `date-fns`, `lottie-react` (only `Pageloader.jsx`, which is dead), `react-day-picker` (only via the commented-out DOB field — see `CF-40`). Also `zod@4`: `Checkout.jsx:27` uses `z.string().email()`, soft-deprecated in v4 in favour of `z.email()`. | Remove all 8; run `npm audit`; migrate the Zod call. | TODO |
...
## 16. Phase 6 — Slice 1: shared module + stripped backends (2026-08-30)

### `DEP-13` ✅ **DONE** — the duplication is gone

`shared/` now holds one copy of `sanitizeError`, `redactQuoted`,
`makeDescribeDuplicate`, `validate`, `rejectNonScalarBody` and
`createWithTransaction`. **86 duplicated lines → 78 lines in one place.**

Each backend keeps a thin file at the original path that re-exports from
`shared/`, so **all 16 import sites were left untouched**.

**The owner's insight, and it was the right call:** the per-service
`DUPLICATE_FIELDS` lists are *supposed* to differ. Admin writes to `product`
and `category`; client writes to `cart` and `wishlist`; neither can violate the
other's constraints. Both copies previously carried all six entries — each
listing constraints it could never hit — which is what made them look like
copies needing synchronisation. `makeDescribeDuplicate(list)` now takes the list
as an argument, so each service declares only its own surface and there is
nothing to keep identical.

⚠️ **Caught during development, not in production:** the first boot failed with
`Cannot find module 'express-validator'` from `shared/validate.js`. Node
resolves dependencies by walking *up* from `shared/`, and the backends' own
`node_modules` sit in their folders, not above it. Fixed by giving `shared/` its
own `package.json` and committed lockfile — **which changes the Render build
command.** See `DEPLOYMENT.md`; get it wrong and both services die on boot.

### `DEP-01` … `DEP-08`, `DEP-10`, `DEP-11` ✅ **DONE** — the backends were React apps

Both were scaffolded with `create-react-app` and never cleaned up. Named
`project1`, shipping `react`, `react-dom`, `react-scripts`, `web-vitals`, four
`@testing-library` packages, a `public/` folder with `index.html` and a favicon,
a `browserslist`, a `react-app` eslint config, and `build`/`test`/`eject`
scripts that cannot work in a server. React was imported **zero** times.

| | before | after |
|---|---|---|
| client-backend | 929 packages, **348 MB** | 127 packages, **20 MB** |
| admin-backend | 918 packages, **344 MB** | 142 packages, **39 MB** |

Also removed: **`crypto@1.0.1`**, the squatted `npm/deprecate-holder` placeholder
— verified inert, `require.resolve('crypto')` returns the built-in;
**`bcryptjs`**, a second hashing library with zero imports beside the `bcrypt`
actually used; and **`pnpm-lock.yaml`**, so one lockfile governs installs rather
than two competing ones. `nodemon` moved to `devDependencies`. `cloudinary`,
`multer` and `multer-storage-cloudinary` were removed from **client-backend
only** — it has no upload routes (verified in its server and router) — and kept
in admin, which does.

**Verified after a full `rm -rf node_modules` and fresh install:** both boot
clean; 35 products / 7 new releases / 6 collections / 4 bestsellers / 8
categories served anonymously; all four auth boundaries still 401; complete
customer journey — signup, login, 2 cookies, verify-token, cart add + update,
order via `withTransaction`, order history, profile PUT, logout; all six admin
endpoints; **an image upload through multer + cloudinary returning 200**; and
order-status validation accepting the enum and rejecting anything else. 9/9
endpoint checks passed on a second clean restart.
...
| Item | Decision | Notes |
|---|---|---|
| **`INFRA-03` — move Render to the owner's own account, make the repo PRIVATE** 🆕 | **TO DO — owner decision, 2026-08-30. Do this BEFORE or DURING the next deploy.** | **The repo is public only as a workaround.** Render is hosted on a **colleague's personal account, under his email**, while the GitHub repo belongs to the owner. Because the two accounts are unrelated, that Render could not pull a private repo — so the repo was made **public** to let it. That single workaround is why `SEC-01` is as serious as it is: `CLOUDINARY_API_SECRET` is retrievable by anyone from commit `8f8cb01` (`git show 8f8cb01:admin-backend/.env` — verified 2026-08-30).<br><br>The owner will open a Render account on **the same email as his GitHub**, which removes the reason for a public repo entirely.<br><br>**⛔ ORDER MATTERS:**<br>**1. Rotate the Cloudinary key FIRST.** Making the repo private does not unring the bell — anyone who already cloned it still holds the key. Only rotation invalidates it. This is `SEC-01`.<br>**2. Create the Render account** on the GitHub-linked email and connect it.<br>**3. Make the repo private.** From here the key cannot be read by anyone new.<br>**4. Purge git history** (`git filter-repo` / BFG) — optional once rotated, since the old key is then worthless. Tidiness, not urgency.<br>**5. Recreate both services** with the settings below.<br>**6. Repoint the domains**, verify, then shut down the colleague's services.<br><br>**Service settings — note these differ from today, because `shared/` lives at the repo root:**<br>`Root Directory` — **blank** (the repo root, so `require('../../shared/...')` resolves)<br>`Build Command` — `cd client-backend && npm ci`<br>`Start Command` — `cd client-backend && npm start`<br>(and the same for `admin-backend`). Env vars per `DEPLOYMENT.md`, including the exact `CORS_ORIGINS` values.<br><br>⚠️ **A shared module is NOT a third service.** It does not run, listen or have a URL — it is files read at boot, like anything in `node_modules`. **No extra Render service, no extra cost.** The only change is which folder Render uploads.<br><br>**For handover:** the client should ultimately own Render, Cloudinary/AWS and the domain, with the developer as collaborator. Today the client's live shop sits in a colleague's personal account — the client cannot manage it and the colleague is on the hook indefinitely. Raise this with them. |
...
| **`DEP-14` — bump `react-router-dom` (2 HIGH advisories, both frontends)** 🆕 | **DEFERRED — owner decision, 2026-08-30: do it AFTER Phase 6 completes.** Not negligence — nothing is being exploited today — but it should not ship to a client without being closed. | **`npm audit` reports 2 high-severity, 14 advisories.** client-frontend is on **7.8.2**, admin-frontend on **7.9.3**; the vulnerable range is `7.0.0 – 7.11.0` and the fix is **7.18.3**. Found while bumping Vite for `CF-48`, not by audit.<br><br>**Most advisories are SSR/RSC-specific and do not apply to these SPAs.** The ones that do: **open redirect via backslash in `<Link>`/`useNavigate`** (CVE-2025-68470 bypass), **open redirect via protocol-relative `//` paths**, **XSS via open redirects**, and **DoS via inefficient route matching**. The open redirect is the one that matters commercially — an attacker can send a customer to a `paisilks.com` link that lands them on a phishing page imitating checkout.<br><br>**Risk assessed as LOW before deferring, and the evidence is worth keeping so it need not be re-derived:** it is a **same-major** bump (7.x → 7.x); both apps use **only the 8 classic APIs** (`useNavigate` ×13, `useLocation` ×7, `Link` ×4, `Routes`, `Route`, `Navigate`, `BrowserRouter`, `useParams`); **zero data-router APIs** (`createBrowserRouter`, `RouterProvider`, `loader`, `action`, `useLoaderData`, `useFetcher`, `Form` — all absent, verified, the earlier `Form`/`action` grep hits were false positives); and **every navigation target is a simple absolute path** (`/`, `/login`, `/shop`, `/checkout`, `/my-orders`, `/about-us`, `/admin-home-page`, `/signup`, `/forgot-password`) with **zero** protocol-relative or backslash paths — which is precisely the behaviour the CVE fix changes, so the one plausible breakage does not exist in this code.<br><br>**When doing it:** `npm install react-router-dom@^7.18.3` in both, rebuild both, then smoke-test routing — storefront pages, product detail via `useParams`, the `/:productId` catch-all (`CB-39`), admin login → dashboard, and `ProtectedAdminRoute`. Reverting is one `npm install react-router-dom@7.8.2` away and the lockfiles record the exact prior state. Estimated **~5 minutes**. |
| **Set cover image — choose which photo represents a product** 🆕 | **DEFERRED — recorded during Phase 6 Slice 2 when `resetPrimaryImageByImageId` was deleted (`AB-30`).** Not requested by the owner; recorded because deleting the half-built helper would otherwise erase the fact that this gap exists. | **Today the cover image is decided once and can never be changed.** `is_primary_image` is set at upload only — `productManager.js:65` and `adminController.js:121` both do `i === 0 ? 1 : 0`, so **whichever file the admin happens to select first becomes the shop's display image forever.** There is no UI to change it (verified: zero matches for primary/cover/thumbnail selection in the admin frontend). To fix a bad cover today the admin must delete the product's images and re-upload them in the desired order.<br><br>**Why it matters:** **9 storefront queries** join `AND pi.is_primary_image = 1` to choose the product's display image — homepage, collections, category pages, bestsellers, new releases, cart, wishlist and order lines all render whatever that flag points at.<br><br>⛔ **BUILD IT AS ONE TRANSACTION, and this is the whole point of the entry.** The operation is two statements: clear `is_primary_image` for every image of the product, **then** set it to 1 for the chosen image. The deleted helper was **only the first half**. Running the clear without the set leaves the product with no primary image, all 9 `LEFT JOIN`s return `NULL` for `image_url`, and **the product silently loses its picture across the entire storefront** — no error, no log, discovered by a customer. Wrap both in `withTransaction`, and re-assert the invariant afterwards: exactly one row per `product_id` with `is_primary_image = 1`.<br><br>**Scope:** `PUT /api/products/:id/primary-image` on admin-backend taking an `image_id`, validating that the image belongs to that product (otherwise one product's photo can be made another's cover); a "Set as cover" control on the image grid in `UpdateProduct.jsx`; and a visible badge marking the current cover so the admin can see what they are changing.<br><br>**Consider a DB-level guarantee:** nothing currently stops two rows for the same product both having `is_primary_image = 1`, in which case the 9 queries pick arbitrarily and the storefront can disagree with itself page to page. Worth checking whether that already exists in production data before building the feature on top of it. |
...
**Slice 1 — shared module + stripped backends ✅ DONE**
- [x] `DEP-13` `shared/` module extracted; 86 duplicated lines → 78 in one place
- [x] `DEP-01` Frontend deps and `public/` removed from both backends *(929→127 and 918→142 packages; 348 MB→20 MB and 344 MB→39 MB)*
- [x] `DEP-02` `bcryptjs` removed · `DEP-03` `crypto` removed · `DEP-05` `nodemon` moved
- [x] `DEP-08` One lockfile per app *(`pnpm-lock.yaml` deleted)*
- [x] `DEP-10` / `DEP-11` unused frontend deps removed *(closed fully in Slice 2)*
- [x] `AF-07` / `CF-*` All 37 hardcoded API URLs replaced with env vars *(done in Phase 0.5; the box was never ticked)*

**Slice 2 — dead code ✅ DONE**
- [x] `CF-50` 19 dead CSS files + dead JS/JSX deleted · `CF-51` **9** unused deps removed
- [x] `AF-37` admin dead files + 7 unreferenced assets deleted
- [x] `CF-34` dead prop plumbing removed from 9 files
- [x] `AF-08` `categoryProducts` write-only state fully removed *(state + prop + both call sites)*
- [x] `AB-29` unused `asyncHandler.js` deleted from **both** backends
- [x] `AB-30` 7 orphaned dbOps methods + 7 SQL strings deleted
- [x] `CB-40` signup/login password field name unified on `passwd`
- [ ] ~~`CF-44`~~ **moved to Slice 4** — not dead code. The `WishList.jsx` implementation is the *better* one (checks `res.ok`, rolls back on failure); deleting it would lose that. Correct fix is to promote it into `CartContext`, which is a behaviour change and belongs with the rest of `CF-17`.

**Slice 3 — server config & operability ⬜ NOT STARTED**
- [ ] `CB-37` / admin twin **hardcoded `PORT`** — `client-backend/src/server.js:109` = `9034`, `admin-backend/src/server.js:108` = `9032`. ⚠️ **Render assigns the port via `process.env.PORT`; both services fail to bind as-is.**
- [ ] `SIGTERM` handler — absent in both
- [ ] `unhandledRejection` handler — absent in both
- [ ] `express.urlencoded` — absent in both
- [ ] Pool error handler — absent in both
- [ ] `/get-order-detils` typo — route `adminRoutes.js:66` + caller `AdminHomePage.jsx:197`

**Slice 4 — frontend leftovers ⬜ NOT STARTED**
- [ ] `CF-48` ngrok host removed from `vite.config.js:16` (`fleshly-succulently-jona.ngrok-free.dev`); Vite bumped
- [ ] `CF-53` Real README written *(currently 1 line)*
- [ ] `CF-44` promote the hardened wishlist removal into `CartContext` *(moved from Slice 2)*
- [ ] `AF-09` / `CF-17` residuals

**Slice 5 — lint & CI ⬜ NOT STARTED**
- [ ] `DEP-09` `npm run lint` passes in both frontends — **currently client 21 errors + 2 warnings, admin 7 errors**. Three of admin's are the shadcn `react-refresh/only-export-components` convention (fixable with `allowConstantExport: true`) and one is `__dirname` in `vite.config.js` being linted with browser globals rather than Node.
- [ ] `DEP-12` CI pipeline live: install → lint → **Linux build** *(no `.github/workflows` exists)*
