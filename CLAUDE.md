...
### `AF-C-FIX` — Crash-on-undefined, admin-frontend · HIGH · **TODO**
...
- [ ] `AF-C-FIX` admin ErrorBoundary mounted in `admin-frontend/admin/src/main.jsx`
- [ ] `AF-C-FIX` `AdminHomePage.jsx:138` guarded — an order missing `product_list` no longer
      silently hides the **entire** order list
- [ ] `AF-C-FIX` swallowed crashes at `AdminHomePage.jsx:91, :120` guarded, and their `.catch`
      blocks no longer discard real errors
- [ ] `AF-C-FIX` no `₹NaN` renders anywhere in the admin panel
